...
class State:
    ...
    def sample_hand(self): # returns a plausible hand for the moving player @ current state
...
class MCTSAgent:
    def __init__(self, player_idx, env: Hanabi):
        ...
        self.player_idx = player_idx #! WOW
        self.env = env
    ...
    def act(self, action):
        if action == 0: self.__play_probably_safe_card(0.7)
        elif action == 1: self.__discard_probably_useless_card(0.0)
        elif action == 2: self.__tell_anyone_about_useful_card()
        elif action == 3: self.__tell_dispensable()
        elif action == 4: self.__tell_randomly()

env = Hanabi(NUM_PLAYERS, verbose=True)
env.reset(0)

agents = [MCTSAgent(i, env) for i in range(NUM_PLAYERS)]

done = False
while not done:
    for a in agents:
        action = a.compute_action(3.0)
        a.act(action)
        if (len(env.deck) == 0 and all(env.played_last_turn)) or \
            sum(len(env.table_cards[k]) for k in COLORS) == 25 or \
            env.err_tk == 3:
            done = True
            break
